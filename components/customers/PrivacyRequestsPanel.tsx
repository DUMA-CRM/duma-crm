...
import { Download, Plus, ShieldCheck } from '@/components/icons';
  ...
  return (
    <section className="rounded-2xl border border-border bg-card shadow-sm p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="font-semibold text-foreground">Privacy requests</h2>
          <p className="mt-1 text-xs text-muted-foreground">Track access, portability, correction, restriction and erasure requests.</p>
        </div>
        {customerId && (
      ...
      <div className="mt-4 space-y-2">
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">No privacy requests recorded.</p>
        ) : (
          data.map((request) => {
            const name = request.customer ? `${request.customer.firstName} ${request.customer.lastName}` : request.customerSnapshot?.name;
            return (
              <div key={request.id} className="rounded-xl border border-border bg-surface-offset p-3">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <p className="text-sm font-semibold text-foreground">
                      {TYPE_OPTIONS.find((option) => option.value === request.type)?.label}
                      {!customerId && name ? ` · ${name}` : ''}
                    </p>
                    <p className="mt-0.5 text-xs text-muted-foreground">
                      Received {formatDate(request.receivedAt)} · due {formatDate(request.dueAt)}
                    </p>
                  </div>
                  <Badge
                    variant={request.status === 'completed' ? 'success' : new Date(request.dueAt) < new Date() ? 'destructive' : 'warning'}
                  >
                ...
                {request.status !== 'completed' && request.status !== 'declined' && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {request.status === 'received' && (
                      <Button variant="outline" size="sm" onClick={() => progress.mutate(request)}>
                        Start work
                      </Button>
                    )}
      ...
      {creating && (
        <Modal title="New privacy request" onClose={() => setCreating(false)}>
          <div className="space-y-4">
            <Select
              ...
              className="w-full"
            />
            <Select value={channel} onValueChange={setChannel} options={CHANNEL_OPTIONS} ariaLabel="Request channel" className="w-full" />
            <textarea
              value={details}
              onChange={(event) => setDetails(event.target.value)}
              maxLength={2000}
              placeholder="What did the customer ask for?"
              className="min-h-28 w-full rounded-lg border border-input bg-field p-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
            />
