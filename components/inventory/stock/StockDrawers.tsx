...
export function EditThresholdDrawer({ item, onClose, onSuccess }: { item: LocationStock; onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <Drawer
      ...
      description={item.stockItem?.name ?? 'Stock'}
      onClose={onClose}
      footer={
        <div className="flex gap-2">
          <Button type="button" variant="outline" className="flex-1" onClick={onClose}>
          ...
          <Button type="submit" form={THRESHOLD_FORM} className="flex-1" disabled={isPending}>
            {isPending ? 'Saving…' : 'Save'}
          </Button>
        </div>
      }
    >
      <form id={THRESHOLD_FORM} onSubmit={handleSubmit} className="space-y-4">
        <div className="flex flex-col gap-1.5">
...
export function RestockDrawer({ item, onClose, onSuccess }: { item: LocationStock; onClose: () => void; onSuccess: () => void }) {
    ...
    <Drawer
      ...
      footer={
        <div className="flex gap-2">
          ...
          <Button type="submit" form={RESTOCK_FORM} className="flex-1" disabled={isPending}>
...
export function LogLossDrawer({
  ...
  return (
    <Drawer
      title="Log Loss"
      description="Write off stock that spoiled, broke or went missing."
      onClose={onClose}
      footer={
        <div className="flex gap-2">
          <Button type="button" variant="outline" className="flex-1" onClick={onClose}>
          ...
          <Button type="submit" form={LOG_LOSS_FORM} className="flex-1" disabled={isPending}>
            {isPending ? 'Logging…' : 'Log Loss'}
          </Button>
        </div>
      }
    >
      <form id={LOG_LOSS_FORM} onSubmit={handleSubmit} className="space-y-4">
        <div className="flex flex-col gap-1.5">
    ...
    <Drawer
      ...
      footer={
        <div className="flex gap-2">
          ...
          <Button type="submit" form={EDIT_ITEM_FORM} className="flex-1" disabled={isPending || !name.trim() || !unit.trim()}>
