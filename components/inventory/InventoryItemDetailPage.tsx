        ...
        <div className="min-w-0 w-full flex justify-between">
          <div>
            ...
            <h1 className="text-lg font-semibold text-foreground truncate">
              {item?.name ?? (itemLoading ? 'Loading…' : 'Inventory item')}
            </h1>
          </div>
          {item && (
            ...
            <section className="rounded-2xl border border-border bg-card p-5">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5 items-end">
                ...
                <Button onClick={() => receive.mutate()} disabled={receive.isPending}>
                  {receive.isPending ? 'Receiving…' : 'Receive containers'}
                </Button>
              </div>
            </section>
          )}
          ...
          <section className="rounded-2xl border border-border bg-card overflow-hidden">
            ...
            {!locationId ? (
              ...
              <div className="py-16">
                <EmptyState
                  icon={Box}
                  title="No stock units"
                  description="Receive a delivery or add a physical container to begin unit tracking."
                />
              </div>
            ) : (
                    ...
                    {units.map((unit) => (
                      <tr key={unit.id} className="border-b border-border/50 last:border-0 hover:bg-surface-offset/50">
                        <td className="px-5 py-3">
                          <Link href={`/inventory/units/${unit.id}`} className="font-medium text-primary hover:underline">
                            {unit.label}
                          </Link>
                        </td>
                        <td className="px-5 py-3 text-muted-foreground">{unit.lotNumber || '—'}</td>
                        <td className="px-5 py-3 text-right tabular-nums">
                          {fmt(unit.remainingQuantity)} / {fmt(unit.initialQuantity)} {unit.unitOfMeasure}
                        </td>
                        <td className="px-5 py-3 tabular-nums">
                          {unit.expiryDate ? new Date(unit.expiryDate).toLocaleDateString('en-GB') : '—'}
                        </td>
                        <td className="px-5 py-3">
                          <Badge
                            variant={
                              unit.status === 'AVAILABLE' || unit.status === 'IN_USE'
                                ? 'success'
                                : unit.status === 'EXPIRED'
                                  ? 'destructive'
                                  : 'muted'
                  ...
                  <div key={movement.id} className="px-5 py-3 flex items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-foreground">
                        {movement.reason?.replaceAll('_', ' ') ?? movement.notes ?? 'Stock movement'}
                      </p>
                    ...
                    <span className={cn('font-semibold tabular-nums', Number(movement.quantity) < 0 ? 'text-destructive' : 'text-success')}>
                      {Number(movement.quantity) > 0 ? '+' : ''}
                      {fmt(movement.quantity)} {movement.unitOfMeasure ?? item?.unit}
                    </span>
