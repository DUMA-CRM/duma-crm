...
import { Download, RotateCcw } from '@/components/icons';
    ...
    <EditorShell eyebrow="Report" title="Refunds" icon={<RotateCcw size={20} />} onClose={() => router.push('/reports/library')}>
      <div className="space-y-5">
        <div className="flex flex-wrap items-end gap-3 rounded-sm border border-rule bg-card shadow-sm p-4">
          ...
          <div>
            <p className="mb-1 text-xs font-semibold text-muted-foreground">Date attribution</p>
            <Select
              value={basis}
              onValueChange={(value) => setBasis(value as 'refund' | 'sale')}
              options={[
                { value: 'refund', label: 'Refund date' },
                { value: 'sale', label: 'Original sale date' },
              ]}
              ariaLabel="Date attribution"
            />
          </div>
          <div>
            <p className="mb-1 text-xs font-semibold text-muted-foreground">Reason</p>
            <Select value={reason} onValueChange={setReason} options={reasons} ariaLabel="Refund reason" />
          </div>
          <Button variant="outline" asChild>
            <a href={refundReportCsvUrl(params)} download>
              <Download size={15} /> Export CSV
            </a>
          </Button>
          <StatCard
            size="sm"
            label="Refunded in period"
            value={`£${Number(data?.totalAmount ?? 0).toFixed(2)}`}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-rule text-left text-xs text-muted-foreground">
                ...
                <th className="p-3">Items and modifiers</th>
              ...
              {isPending ? (
                ...
                data.data.map((refund) => (
                  <tr key={refund.id} className="border-b border-rule last:border-0">
                    ...
                    <td className="p-3">
                      <p className="font-medium">{refund.order.location?.name ?? 'Unknown location'}</p>
                      <p className="text-xs text-muted-foreground">#{refund.orderId.slice(0, 8).toUpperCase()}</p>
                    </td>
                    <td className="p-3">
                      {refund.lines?.map((line) => (
                        <p key={line.id}>
                          {line.quantity}× {line.name} <span className="text-muted-foreground">£{Number(line.amount).toFixed(2)}</span>
                        </p>
                      ))}
                    </td>
                    <td className="p-3 capitalize">{refund.reason.replaceAll('_', ' ')}</td>
                    <td className="p-3 text-right font-bold text-destructive">−£{Number(refund.amount).toFixed(2)}</td>
                  </tr>
                ))
              ) : (
                <tr>
        ...
        <p className="text-xs text-muted-foreground">
          Refund-date view supports cash/accounting reconciliation. Sale-date view restates the original sales period after refunds.
        </p>
      </div>
    </EditorShell>
  );
