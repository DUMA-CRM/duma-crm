...
import { CATEGORY_COLORS, CATEGORY_LABELS, CATEGORY_OPTIONS, FormActions, inputClass, labelClass, selectClass } from '@/components/menu/shared';
...
function ItemModifiersEditor({ menuItemId }: { menuItemId: string }) {
  ...
  const { data: attached = [] } = useQuery({ queryKey: ['menu-item-modifiers', menuItemId], queryFn: () => getMenuItemModifiers(menuItemId) });
  ...
  return (
    ...
    >
      <div>
        ...
        <input value={name} onChange={(e) => setName(e.target.value)} required minLength={2} placeholder="Flat White" className={inputClass} autoFocus />
      </div>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input value={price} onChange={(e) => setPrice(e.target.value)} required pattern="^\d+(\.\d{1,2})?$" placeholder="3.20" className={inputClass} />
        </div>
      </div>
      <div>
        <label className={labelClass}>Description</label>
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} placeholder="Optional" className={inputClass + ' h-auto py-2 resize-none'} />
      </div>
      <div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="sticky top-0 z-10">
              <tr className="border-b border-border bg-muted">
                <th className="px-5 py-3.5 text-left text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Item</th>
                <th className="px-5 py-3.5 text-left text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Category</th>
                <th className="px-5 py-3.5 text-right text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Price</th>
              ...
              {isLoading ? (
                ...
                items.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <span className={cn('inline-flex items-center px-2.5 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wide', CATEGORY_COLORS[item.category])}>
                        {CATEGORY_LABELS[item.category]}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-right tabular-nums font-semibold text-foreground">{money(item.price)}</td>
                    <td className="px-5 py-3.5">
                      <span
                        className={cn(
                          'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] font-bold uppercase tracking-wide',
                          item.isAvailable ? 'bg-success/10 text-success border-success/30' : 'bg-muted text-muted-foreground border-border',
                        )}
                      >
                        <span className={cn('w-1.5 h-1.5 rounded-full shrink-0', item.isAvailable ? 'bg-success' : 'bg-muted-foreground')} />
