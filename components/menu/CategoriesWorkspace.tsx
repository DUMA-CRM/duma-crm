...
import { createMenuCategory, getMenuCategories, reorderMenuCategories, updateMenuCategory } from '@/lib/modules/catalog/client';
...
export function CategoriesWorkspace() {
  ...
  const create = useMutation({
    mutationFn: () =>
      createMenuCategory({ tenantId: tenantId!, name, description: description || undefined, imageUrl: imageUrl || undefined }),
    onSuccess: () => {
      setName('');
      setDescription('');
      setImageUrl('');
      ...
      {!tenantId ? (
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <section className="overflow-hidden rounded-lg border border-rule/65 bg-card">
            <div className="border-b border-rule/55 bg-band/55 px-4 py-3">
              <h2 className="font-semibold">Customer-facing sections</h2>
              <p className="mt-1 text-xs text-muted-foreground">The same order is used in POS and QR menus.</p>
            </div>
            {categories.isPending ? (
              ...
              <div className="divide-y divide-rule/45">
                    ...
                    {editingId === category.id ? (
                      <form
                        ...
                        onSubmit={(event) => {
                          ...
                          update.mutate({ id: category.id, data: { name: editName, description: editDescription || null } });
                        }}
                      >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-0.5 truncate text-xs text-muted-foreground">
                            {category.description || `${category.itemCount} ${category.itemCount === 1 ? 'item' : 'items'}`}
                          </p>
                        </div>
                        <Button
                          variant="ghost"
                        ...
                        <Button
                          ...
                          onClick={() => update.mutate({ id: category.id, data: { isActive: !category.isActive } })}
                        >
                          {category.isActive ? <Eye /> : <EyeOff />}
                ...
                {!categories.data?.length && (
                  <EmptyState icon={LayoutGrid} title="No categories" description="Create the first section for this menu." />
                )}
              </div>
            )}
          </section>
          <section className="rounded-lg border border-rule/65 bg-card p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Add a section without a code release.</p>
            <form
            ...
            >
              ...
              <Input
                ...
                maxLength={500}
                placeholder="Fresh bakes and sweet treats"
              />
              <Input
                label="Image URL"
                type="url"
              ...
              <Button type="submit" className="w-full" disabled={!name.trim() || create.isPending}>
                {create.isPending ? <Loader2 className="animate-spin" /> : <Plus />}Create category
              </Button>
            </form>
          </section>
        </div>
