...
function ClockEntryRow({ entry, canEdit, onSaved }: { entry: Shift; canEdit: boolean; onSaved: (shift: ClockResponse) => void }) {
  const day = toDateInput(new Date(entry.clockedIn));
  ...
  const dirty = inTime !== original.in || outTime !== original.out;
  ...
  const minutes = entry.clockedOut
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {dirty && (
          <Button size="xs" onClick={() => adjust.mutate()} disabled={adjust.isPending} className="ml-auto">
            {adjust.isPending ? 'Saving…' : 'Save time'}
          </Button>
        )}
      </div>
      {adjust.error && <p className="mt-1.5 text-xs text-destructive">{(adjust.error as Error).message}</p>}
    </div>
  );
}
...
export function ShiftRecordDrawer({
  target,
  defaultLocationId,
  locations,
  staff,
  ...
  return (
    <Drawer
      title={editing ? 'Shift record' : 'Create a time record'}
      description={
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-3.5">
          ...
          <Field icon={CalendarDays} label="Assignment date">
            <DatePicker value={date} onValueChange={setDate} required aria-label="Assignment date" />
          </Field>
        ...
        <Card title="Time records">
          <div className="space-y-4">
            <div>
              ...
              <Hint tone="success">{fmtDuration(plannedMinutes)} will be added to the time sheet.</Hint>
            </div>
