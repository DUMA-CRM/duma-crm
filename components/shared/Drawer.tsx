  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div
        ...
        tabIndex={-1}
        className={cn(
          'relative flex h-full w-full max-w-xl flex-col border-l border-border bg-card shadow-2xl outline-none',
          'duration-200 animate-in slide-in-from-right',
          className,
        )}
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-border px-6 pb-4 pt-5">
        ...
        {footer && <div className="shrink-0 border-t border-border bg-surface-offset/40 px-6 py-4">{footer}</div>}
      </div>
    </div>,
    document.body,
  );
