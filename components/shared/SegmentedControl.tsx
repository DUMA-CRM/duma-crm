      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={cn(
              'shrink-0 flex items-center justify-center gap-1.5 rounded-lg text-xs font-bold tracking-widest uppercase transition-colors',
              size === 'lg' ? 'h-9' : 'h-7',
              iconOnly ? (size === 'lg' ? 'w-9' : 'w-7') : 'px-4',
              active ? 'bg-card text-primary shadow-sm' : 'bg-transparent text-muted-foreground hover:text-foreground',
            )}
          >
