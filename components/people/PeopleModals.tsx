...
import { Avatar, EMPLOYMENT_CONFIG, EMPLOYMENT_TYPES, ROLE_CONFIG, ROLES, SCOPES, inp, lbl, sel, toDateInput } from './shared';
    ...
    <form onSubmit={(e) => { e.preventDefault(); mutate(); }} className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          <input type={showPw ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} placeholder="Min 8 characters" className={inp + ' pr-10'} />
...
export function EditStaffModal({ member, locations, onClose }: { member: StaffProfile; locations: { id: string; name: string }[]; onClose: () => void }) {
  const qc = useQueryClient();
          ...
          <div className="flex flex-col gap-1.5 mt-1">
            {locations.map((loc) => (
              <label key={loc.id} className="flex items-center gap-2.5 cursor-pointer select-none px-3 py-2 rounded-lg hover:bg-muted transition-colors">
                <input type="checkbox" checked={selectedLocs.includes(loc.id)} onChange={() => toggleLoc(loc.id)} className="w-4 h-4 rounded accent-primary" />
...
export function EnrollEmployeeModal({ member, onClose }: { member: StaffProfile; onClose: () => void }) {
  ...
  const { mutate, isPending, error } = useMutation({
    mutationFn: () => {
      const payload: CreateEmployeePayload = { userId: member.userId, jobTitle, employmentType, startDate: new Date(startDate).toISOString() };
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); mutate(); }} className="space-y-4">
      ...
      <div>
        ...
        <input value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} required placeholder="Store Manager" className={inp} autoFocus />
      </div>
      <div>
...
export function EditEmployeeModal({ employee, name, onClose }: { employee: HrEmployee; name: string; onClose: () => void }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); save.mutate(); }} className="space-y-4">
      ...
      <div className="flex gap-2 pt-1">
        <button type="button" onClick={() => offboard.mutate()} disabled={offboard.isPending} className="h-10 px-3 border border-destructive/30 text-destructive rounded-xl text-sm font-medium hover:bg-destructive/10 transition-colors disabled:opacity-60 flex items-center gap-1.5">
          <UserMinus size={15} />
          {offboard.isPending ? 'Offboarding…' : 'Offboard'}
        </button>
        <button type="submit" disabled={save.isPending} className="flex-1 h-10 bg-primary hover:bg-primary-hover text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-60">
