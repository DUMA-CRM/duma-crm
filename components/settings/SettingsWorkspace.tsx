...
export function SettingsWorkspace({ tab }: { tab: SettingsTab }) {
  ...
  // Connectors are franchise-owner+ only (mirrors the old nav gating), so the tab
  // never appears for a role that cannot use it.
  ...
  // A stale link to connectors without the role falls back to general.
  const active: SettingsTab = tab === 'connectors' && !showConnectors ? 'general' : tab;
