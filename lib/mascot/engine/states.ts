...
export type StateId =
  ...
  | 'swirl'
  /** DUMA: attente qui garde le visage. Hors `SEQUENCE`, comme `swirl`. */
  | 'pondering'
  /** DUMA: attente qui lit. Hors `SEQUENCE`, comme `swirl`. */
  | 'scanning'
...
export const STATES: StateDef[] = [
  ...
  {
    /**
     * DUMA DEVIATION — a waiting state that keeps the mascot's face.
     *
     * The engine's own waiting pose is `thinking`, which is measured and lovely:
     * the ball becomes the middle of three pulsing dots. But it means the mascot
     * *stops being a mascot* for as long as the wait lasts, and a wait is exactly
     * when a person is watching it — so the character hides at the only moment it
     * had a job to do. That is what this replaces.
     *
     * Built out of `swirl`'s vocabulary rather than `orbit`'s, and the two flags
     * are the whole reason:
     *
     *   - `baseBody` keeps the chosen body, so the mascot's silhouette does not
     *     become a spinning triangle;
     *   - `baseFace` keeps the resting face, so it goes on blinking, drifting and
     *     following the pointer while it works. `orbit` sends the eyes racing
     *     round the sphere, which is the same hiding problem in a different form.
     *
     * Where it departs from `swirl` is that it **loops**. Swirl is an entrance: its
     * rings fade out by 1.2s so the view can settle on a clean frame. A wait has no
     * known length, so these fade in and then stay, and `arcRender` is periodic in
     * `t` — the rings orbit for as long as they are asked to, with no cycle to
     * schedule and nothing to resolve.
     *
     * Deliberately NOT in `SEQUENCE`: it is an interface state, not a catalogue
     * animation, exactly as `swirl` is not.
     */
    id: 'pondering',
    duration: 2.6,
    morph: 0.4,
    ...
    baseFace: true,
    baseBody: true,
    // No blink on the way in: the body does not change shape here, and a blink
    // exists to mask a shape morph. Blinking for nothing reads as a flinch.
    blinkIn: false,
    pose: (t) =>
      base({
        // Three of `orbit`'s six measured rings: half the bouquet reads as one,
        // it is half the arcs to rasterise per frame, and six around a ball that
        // keeps its face crowds the eyes.
        //
        // Every OTHER ring, not the first three. The seeds are ordered, so a
        // contiguous slice takes a contiguous slice of everything they carry:
        // `tilt` is `i/6` of a half-turn and `hue` is `i/6` of the palette arc, so
        // rings 0-2 all lean the same way in the same three warm hues. A stride of
        // two keeps the measured spread on both axes.
        arcs: RINGS.filter((_, i) => i % 2 === 0).map((s, i) => ({
          id: `pd${i}`,
          seed: s,
          t,
          // They enter one after another and then hold. Staggered by less than
          // swirl's entrance, which had a whole block to play with; here the rings
          // are the answer to "is it working", so they cannot take a second to
          // say so.
          opacity: clamp((t - i * 0.07) / 0.2)
        }))
      })
  },
  ...
  {
    /**
     * DUMA DEVIATION — the second half of a wait, where `pondering` is the first.
     *
     * A request that runs long needs the mascot to be *doing* more than one thing,
     * or the wait stops reading as progress and becomes a loop playing. But every
     * pose that would obviously vary it — `orbit`, `burst`, `comet`, the engine's
     * `thinking` — varies it by taking the face away, which is the one thing a
     * waiting mascot must not do.
     *
     * So this varies the gaze instead of the silhouette: the eyes sweep the width
     * of the sphere and back, narrowed, which reads as reading something. Paired
     * with `pondering`'s orbiting rings it gives a wait two clearly different
     * gestures, both of them still a mascot with a face.
     *
     *   - `baseBody` keeps the chosen body, so the two beats share a silhouette and
     *     the change between them is the gesture rather than a costume swap.
     *   - `baseFace` is **false**, unlike `pondering`: the whole pose *is* the
     *     sweep, so it must not be replaceable by a resting expression, and pointer
     *     tracking has to stand down for it (the engine drops tracking on any state
     *     without the resting face, which is exactly right here — the mascot is
     *     looking at the work, not at you).
     *
     * Deliberately outside `SEQUENCE`, as `swirl` and `pondering` are: an interface
     * state, not a catalogue animation.
     */
    id: 'scanning',
    duration: 2.2,
    morph: 0.35,
    ...
    blinkIn: false,
    pose: (t) => {
      // One sweep across and back per period. Sine rather than a ramp so it eases
      // at each end, which is where a reader's eye rests on a line.
      const sweep = Math.sin((t / 2.2) * TAU)
      return base({
        gaze: {
          /*
           * +-16deg, and the bound is eye SIZE rather than reach.
           *
           * The eyes are painted on a sphere, so an eye's apparent area is the z of
           * its own normal: sweep further and the far eye foreshortens. Measured
           * against the resting pose either side of a wait, which is what a reader
           * compares this against:
           *
           *   +-20deg, 0.20 x 0.34 eyes -> 63% of resting's apparent eye area
           *   +-18deg, 0.21 x 0.41      -> 82%
           *   +-16deg, 0.21 x 0.42      -> 86%
           *
           * The first was the original pick and it read exactly as the defect it
           * looks like: the mascot's eyes shrinking whenever it got busy. 16deg is
           * still `sin 16 = 0.28` of the ball's radius of travel, which is several
           * times the eye's own width — the sweep loses nothing anyone can see.
           */
          yaw: sweep * 16,
          // Dips as it crosses the middle, so the sweep is an arc rather than a
          // slide — the shape a pair of eyes actually makes crossing a line.
          pitch: 7 - Math.abs(sweep) * 4,
          // The measured head tilt is the mascot's signature and does not move.
          roll: REST_GAZE.roll
        },
        split: EYE_SPLIT,
        // A hair narrower than the resting 0.21 x 0.44, which is as far as
        // "concentrating" is worth taking: any flatter and it stops reading as focus
        // and starts reading as an eye that got smaller.
        eyes: pair(0.21, 0.42)
      })
    }
  },
