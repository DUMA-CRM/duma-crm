...
/**
 * DUMA DEVIATION — the one place this vendored file departs from upstream.
 *
 * Upstream measured a complete hue wheel off the reference video, and the ring
 * seeds below still carry those measurements: `hue` spreads six rings evenly
 * around 360deg, `hueSpan` is how far one ring's own gradient travels. Rendered
 ...
 *
 * Saturation and lightness are pulled slightly in from the measured band (0.55 /
 * 0.62) for the same reason as the hue fold: the product's colours are muted,
 * and mid-lightness is what lets one ring read on both the porcelain field and
 * the near-black night surface without a second palette.
 */
function wheel(hue: number, s = 0.5, l = 0.6): string {
  const h = ((HUE_ORIGIN + hue * HUE_FOLD) % 360 + 360) % 360
