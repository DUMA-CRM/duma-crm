...
export interface LeaveType { id: string; name: string; isPaid: boolean; requiresApproval: boolean; defaultAllowanceDays?: string | null }
export interface LeaveEntitlement { id: string; year: number; totalDays: string; usedDays: string; leaveType: LeaveType }
export interface LeaveRequest {
  id: string; userId: string; startDate: string; endDate: string; totalDays: string;
  status: 'pending' | 'approved' | 'declined' | 'cancelled'; notes?: string | null; reviewNotes?: string | null;
  leaveType: LeaveType; employee?: { id: string; name: string; email: string } | null; createdAt: string;
}
export type AttendanceStatus = 'full' | 'partial' | 'missed' | 'no_shift' | 'scheduled' | 'leave';
export interface AttendanceDay { date: string; status: AttendanceStatus; plannedMinutes: number; workedMinutes: number; leaveName?: string | null }
...
export interface HelpdeskTicket {
  id: string; subject: string; category: TicketCategory; priority: TicketPriority; status: TicketStatus;
  createdBy: string; assignedTo?: string | null; createdAt: string; updatedAt: string;
  employee?: { id: string; name: string; email: string }; assignee?: { id: string; name: string } | null;
  messages?: { id: string; authorId: string; authorName: string; body: string; internal: boolean; createdAt: string }[];
}
export interface EmployeeDocument { id: string; title: string; documentType: string; reference?: string | null; issuedAt?: string | null; expiresAt?: string | null; notes?: string | null }
...
export const submitLeaveRequest = (data: { leaveTypeId: string; startDate: string; endDate: string; partialDay?: 'none' | 'start' | 'end'; notes?: string }) =>
...
export const getEmployeeAttendance = (userId: string, from: string, to: string) => apiFetch<AttendanceDay[]>(`/hr/attendance/${userId}?from=${from}&to=${to}`);
...
export const replyTicket = (id: string, message: string, internal = false) => apiFetch<{ success: boolean }>(`/helpdesk/${id}/messages`, { method: 'POST', body: JSON.stringify({ message, internal }) });
...
export const addEmployeeDocument = (data: { userId: string; title: string; documentType: string; reference?: string; issuedAt?: string; expiresAt?: string; notes?: string }) =>
