...
export const createCatalogOption = (data: { menuItemId: string; name: string; sortOrder?: number }, tenantId?: string) =>
  apiFetch<CatalogOption>(`/catalog/options${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'POST', body: JSON.stringify(data) });

export const updateCatalogOption = (id: string, data: Partial<Pick<CatalogOption, 'name' | 'sortOrder'>>, tenantId?: string) =>
  apiFetch<CatalogOption>(`/catalog/options/${id}${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'PATCH', body: JSON.stringify(data) });
...
export const createCatalogOptionValue = (optionId: string, data: { label: string; sortOrder?: number }, tenantId?: string) =>
  apiFetch<CatalogOptionValue>(`/catalog/options/${optionId}/values${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'POST', body: JSON.stringify(data) });

export const updateCatalogOptionValue = (id: string, data: Partial<Pick<CatalogOptionValue, 'label' | 'sortOrder'>>, tenantId?: string) =>
  apiFetch<CatalogOptionValue>(`/catalog/option-values/${id}${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'PATCH', body: JSON.stringify(data) });

export const deleteCatalogOptionValue = (id: string, tenantId?: string) =>
  apiFetch<{ deleted: true }>(`/catalog/option-values/${id}${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'DELETE' });

export const createCatalogVariant = (data: CatalogVariantPayload, tenantId?: string) =>
  apiFetch<CatalogVariant>(`/catalog/variants${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'POST', body: JSON.stringify(data) });

export const updateCatalogVariant = (id: string, data: Partial<Omit<CatalogVariantPayload, 'menuItemId'>>, tenantId?: string) =>
  apiFetch<CatalogVariant>(`/catalog/variants/${id}${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'PATCH', body: JSON.stringify(data) });
...
export const setCatalogVariantLocation = (
  ...
  tenantId?: string,
) => apiFetch<CatalogVariantLocation>(`/catalog/variants/${variantId}/locations/${locationId}${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'PUT', body: JSON.stringify(data) });
...
export const createCatalogDiscount = (data: CatalogDiscountPayload, tenantId?: string) =>
  apiFetch<CatalogDiscount>(`/catalog/discounts${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'POST', body: JSON.stringify(data) });

export const updateCatalogDiscount = (id: string, data: Partial<CatalogDiscountPayload>, tenantId?: string) =>
  apiFetch<CatalogDiscount>(`/catalog/discounts/${id}${tenantId ? `?tenantId=${encodeURIComponent(tenantId)}` : ''}`, { method: 'PATCH', body: JSON.stringify(data) });
