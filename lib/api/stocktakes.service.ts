...
export const getStockBatches = (params: { locationId?: string; stockItemId?: string; expiringWithinDays?: number } = {}) => {
  const q = new URLSearchParams();
  if (params.locationId) q.set('locationId', params.locationId);
  if (params.stockItemId) q.set('stockItemId', params.stockItemId);
  if (params.expiringWithinDays !== undefined) q.set('expiringWithinDays', String(params.expiringWithinDays));
  const qs = q.toString();
  return apiFetch<StockBatch[]>(`/stock-batches${qs ? `?${qs}` : ''}`);
};

export const createStockBatch = (data: {
  locationId: string;
  stockItemId: string;
  quantity: number;
  expiryDate: string;
  ...
  notes?: string;
}) => apiFetch<StockBatch>('/stock-batches', { method: 'POST', body: JSON.stringify(data) });

export const updateStockBatch = (id: string, data: { quantity?: number; expiryDate?: string; notes?: string }) =>
  apiFetch<StockBatch>(`/stock-batches/${id}`, { method: 'PATCH', body: JSON.stringify(data) });

export const deleteStockBatch = (id: string) => apiFetch<{ success: boolean }>(`/stock-batches/${id}`, { method: 'DELETE' });
